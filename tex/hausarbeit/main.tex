...
Weiterhin ist es möglich den ZPU mit einem eingebetteten Betriebssystem zu betreiben. Das \textit{eCos}\footnote{eCos: Real-time OS for embedded Applications (weitere Informationen unter \cite{wp_eCos}}-Embbedded Operating System wird vom ZPU unterstützt. 
...
\subsection{IOCTRL}
=======
Die Funktion versucht zunächst die PCI-Funktion zu aktivieren und liest anschließend die physikalische Adresse des Speichers und dessen Größe vom Speicher des Geräts. Dies geschieht mittels \texttt{pci\_enable\_device(dev)}. 

% Methode weiter beschreiben?! Abkürzen?

Nach der Initialisierung der FIFOs und dem Registrieren des Interrupt-Handlers in der Probing-Funktion, muss noch der Interrupt-Handler registriert werden. Das Listing \ref{modzpu_prob_requestirq} zeigt das Registrieren des Interrupt-Handlers in der Probing-Funktion. 

\begin{lstlisting}[language=C, firstnumber=142,label=modzpu_probe_requestirq, caption=Interrupt-Handler registrieren]
...
Beim Registrieren wird die Funktion \texttt{myirq\_handler} übergeben. 
...
Die Lese - und Schreibzugriffe auf, bzw. von der ZPU, sind in Abbildung \ref{zpu_io} veranschaulicht. Das Treibernmodul kann aus dem Kernel Daten über das Eingabe-FIFO schreiben und über das Ausgabe-FIFO Daten von der PCI-Hardware entgegen nehmen.
