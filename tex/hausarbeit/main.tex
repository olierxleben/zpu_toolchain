...
Der Treiber für das Embedded System stellt die zentrale Steuerungssoftware für Hostsystem und für Treiber dar. Dieser Abschnitt gibt einen Überblick über das Treibermodul und stellt die wichtigsten Funktionen im Modul vor. Zur Übersichtlichkeit wurden Funktionen für den PCI-Treiber in mehrere Dateien aufgesplittet. Das Treibermodul ist wie folgt aufgebaut:
...
\subsection{Interrupt-Behandlung}

\subsubsection{Ermittlung der Interruptnummer}
Wie im Listing \ref{modzpu_initfunc} zu sehen, wird zum probing des PCI-Geräts die Funktion \texttt{mypci\_probe} zugewiesen. Probing dient der Ermittlung der Interruptnummer durch Testen der Hardware (Probing, Vgl. siehe \cite{script_hwstp}, Sete 235 ff.). % TODO: Code?!

\begin{lstlisting}[language=C, firstnumber=98, label=modzpu_mypci_probe, caption=Probing Funktion]
static int mypci_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	int r;
...
% Methode weiter beschreiben.

Nach Initialisierung der FIFOs und dem Registrieren des Interrupt-Handlers 
